    ...
    parameter['batteries'] = [
        {
         'name': 'EV0',
        'capacity': 324,
         'efficiency_charging': 0.99,
         'efficiency_discharging': 0.99,
         'power_charge': 300,
         'power_discharge': 300,
         'maxS': 300,
...
def test_default_parameter():
    ...
    parameter['site']['export_max'] = 200000000 # kW
    parameter['site']['demand_periods_prev'] = {0:0,1:0,2:0} # kW peak previously set for periods 0-offpeak, 1-midpeak, 2-onpeak
    parameter['site']['demand_coincident_prev'] = 0 # kW peak previously set for coincident
    parameter['site']['input_timezone'] = -5 # Timezone of inputs (in hourly offset from UTC)
    parameter['site']['local_timezone'] = 'Colombia/Bogotá' # Local timezone of tariff (as Python timezone string)
    parameter['controller'] = {}
    parameter['controller']['timestep'] = 60*60 # Controller timestep in seconds
    parameter['controller']['horizon'] = 24*60*60 # Controller horizon in seconds
    parameter['controller']['solver_dir'] = 'solvers' # Controller solver directory
...
def ts_inputs(parameter={}, load='Flexlab', scale_load=4, scale_pv=4):
    ...
    data['tariff_energy_map'] = data['tariff_energy_map'].mask((data.index.hour>=10) & (data.index.hour<13), 0)
    data['tariff_energy_map'] = data['tariff_energy_map'].mask((data.index.hour>=17) & (data.index.hour<19), 2)
...
def ts_inputs_ev_schedule(parameter, data):
    ...
    for b in range(len(parameter['batteries'])):
        ...
        var_avail = pd.read_csv('H:\\My Drive\\Articulos tesis\\DESARROLLO\\Ob2\\OUTCOMES\\25-04-2025\\disponibilidad_cada_5min.csv', delimiter=";", encoding='utf-8')
        ...
        energy_con= pd.read_csv('H:\\My Drive\\Articulos tesis\\DESARROLLO\\Ob2\\OUTCOMES\\25-04-2025\\energia_cada_5min.csv', delimiter=";", encoding='utf-8')
        ...
        demand_ext = energy_con['Bus_{!s}'.format(b)].dropna().to_numpy() * 12
