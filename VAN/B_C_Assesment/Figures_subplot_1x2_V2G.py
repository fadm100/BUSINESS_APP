...
# df_L2_L3_PV40k_TF_SB = pd.read_csv(filePath, sep=',')
...
# df_L3_PV40k_TF_SB_DF = pd.read_csv(filePath, sep=',')
...
# df_total = pd.concat([df_L2_L3_PV40k_TF_SB, df_L2_PV4k, df_L2_PV4k_TF, df_L2_PV4k_TF_SB, df_L3_PV40k_TF_SB_DF, df_L2_TF_SB], ignore_index=True)
...
# graficar(df_L2_L3_PV40k_TF_SB, df_L3_PV40k_TF_SB_DF)
...
filePath = 'H:\\My drive\\Articulos tesis\\DESARROLLO\\Ob2\\OUTCOMES\\25-04-2025\\All.csv'
df_L2_L3_PV40k_TF_SB = pd.read_csv(filePath, sep=',')

filePath = 'H:\\My drive\\Articulos tesis\\DESARROLLO\\Ob2\\OUTCOMES\\15-11-2024\\fast\\L3_PV4k_TF_SB_DF.csv'
df_L3_PV4k_TF_SB_DF = pd.read_csv(filePath, sep=',')
...
df_L2_L3_PV40k_TF_SB = Add_Investment(df_L2_L3_PV40k_TF_SB)
df_L2_L3_PV40k_TF_SB['NPV_final'] = df_L2_L3_PV40k_TF_SB['NPV'].apply(lambda x: x[-2] if len(x) >= 2 else None)
# Guardar el resultado si lo necesitas
df_L2_L3_PV40k_TF_SB.to_csv('archivo_modificado.csv', index=False)
df_L3_PV4k_TF_SB_DF = Add_Investment(df_L3_PV4k_TF_SB_DF)
df_L3_PV4k_TF_SB_DF['NPV_final'] = df_L3_PV4k_TF_SB_DF['NPV'].apply(lambda x: x[-2] if len(x) >= 2 else None)


########################################################

def graficar(df_left):
    for df in [df_left]:
    ...
    # --- LEFT PLOTS ---
    for _, row in df_left.iterrows():
        axes[0].plot(row['vida_util'], row['NPV'], label=f"In {row['income']}, DR {row['tasa_descuento']:.0%}")

    axes[0].set_ylabel('VPN (USD)', fontsize=14, fontweight='bold')
    axes[0].set_xlabel('Año\na)', fontsize=14, fontweight='bold')
    axes[0].legend(loc='center left', bbox_to_anchor=(1, 0.5), fontsize=8, title_fontsize='13', frameon=True)
    ...
    for idx, color, label in zip([mejor_idx, peor_idx], ['green', 'red'], ['Mejor VPN', 'Peor VPN']):
        ...
        axes[1].bar(range(len(flujo)), flujo,
                      label=f'{label}: {df_left["NPV_final"].iloc[idx]:.2f}',
                      alpha=0.7, width=0.4, align='center' if idx == mejor_idx else 'edge', color=color)

    axes[1].set_xlabel('Año\nb)', fontsize=14, fontweight='bold')
    axes[1].set_ylabel('Flujo de Caja (USD)', fontsize=14, fontweight='bold')
    axes[1].legend(fontsize=12, title_fontsize='13', loc='best', frameon=True)
