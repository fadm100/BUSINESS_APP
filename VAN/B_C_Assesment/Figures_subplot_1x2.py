...
filePath = 'H:\\My drive\\Articulos tesis\\DESARROLLO\\Ob2\\OUTCOMES\\15-11-2024\\L2_L3_PV40k_TF_SB.csv'
df_L2_L3_PV40k_TF_SB = pd.read_csv(filePath, sep=',')
...
def graficar(df_left):
    ...
    # --- LEFT PLOTS ---
    for _, row in df_left.iterrows():
        axes[0].plot(row['vida_util'], row['NPV'], label=f"DI {row['crecimiento_demanda']:.0%}, DR {row['tasa_descuento']:.0%}")
